...
import { Bed, Bath, MapPin, ArrowUpRight, Home } from 'lucide-react';
import type { PublicVacancy } from '@/queries/vacancies';

export const VacancyCard = ({ vacancy, onApply }: { vacancy: PublicVacancy; onApply: () => void }) => {
  return (
    <motion.div
      whileHover={{ y: -6 }}
    ...
    >
      ...
      <div className="p-6">
        ...
        <button
          type="button"
          onClick={onApply}
          className="group/btn w-full mt-5 py-2 bg-slate-900 text-white rounded-md font-medium text-sm hover:bg-slate-800 transition-colors inline-flex items-center justify-center gap-1.5"
        >
          Apply Now
